...
def fetching_data(idx, pk, cid, df, new_csv):
    ...
    for img_info in imgs:
        ...
        with open(f'img_spotSeq_{pk}_{img_idx}.jpg', 'wb') as f:
            f.write(img_res.content)
            img_json.append({f'pk_{img_idx}': f'img_pk_{pk}_{img_idx}.jpg'})

        # 이미지를 파일로 만드는 로직.(파일명에 네이버 cid 이용 )
        try:
            if np.isnan(cid):
                continue
            with open(f'img_cid_{int(cid)}_{img_idx}.jpg', 'wb') as f2:
                f2.write(img_res.content)
                img_json.append({f'cid_{img_idx}': f'img_cid_{int(cid)}_{img_idx}.jpg'})
        except Exception:
            # 매칭되는 cid 없으면 무시.
            pass
    ...
    df.loc[idx, 'sfiInfo'] = json.dumps(facility_infos)
    df.loc[idx, 'img_info'] = json.dumps(img_json)
